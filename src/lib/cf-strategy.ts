...
/** Prefer jsDelivr (often ahead of raw CDN lag), then GitHub raw. */
...
/** Canonical display / fallback pointer (raw path; mirrors always exist). */
export const SPRAT_SOURCE_URL = SPRAT_SOURCE_URLS[1];

export const CF_STRATEGY_SCHEMA = "twzrd.cf_strategy/v1" as const;

export type CfSupplyLane = {
  id: string;
  name: string;
  status: string;
  detail: string;
...
export type CfStrategy = {
  schema: typeof CF_STRATEGY_SCHEMA;
  source: string;
  ...
  fetched_at: string;
  live_source: boolean;
  thesis: { headline: string; subcopy: string };
  decision: {
    summary: string;
    pick: string;
    ship_now: string[];
    do_not: string[];
    if_spike_later?: unknown;
    github_agents_context?: Record<string, string>;
    signal_b_probe_correction?: unknown;
  };
  agent?: {
    name: string;
    codename: string;
  ...
  signals: { A: CfSignal; B: CfSignal };
  posture: {
    ship_now: string[];
    hold: string[];
    ready_not_shipped: string[];
    guardrail: string;
  };
  company_sentence: string;
  not_this_board: string;
  routing: {
    cf_strategy: string;
    execution: string;
    product: string;
  };
  sprat_mirror?: Record<string, string>;
  external?: Record<string, string>;
};

/** Baked SPRAT extract — used when live fetch fails. Keep strategy content current. */
const EMBEDDED: Omit<CfStrategy, "fetched_at" | "live_source"> = {
  ...
  thesis: {
    ...
    subcopy:
      "Cloudflare owns agent distribution. Solana owns settlement speed. SPRAT by TWZRD is the trust + x402 facilitator middle.",
  },
  decision: {
    summary:
      "Hold engineering on Agents MCP SVM. Flood live x402-hono Solana path. MCP-SVM is watch + optional read-only spike only — not build-now.",
    pick: "brief+hold",
    ship_now: [
    ...
    if_spike_later: {
      ...
      preferred_ship_shape_if_go:
        "Companion package first (usable without CF merge) + optional upstream PR sketch",
    },
    github_agents_context: {
      "596":
        "closed NOT_PLANNED — pure Solana MCP enhancement rejected once",
      "793":
        "closed completed — v2 migrate; body mentioned future Solana; stopped at EVM",
      "1863":
        "open bug — docs claim multi-chain; code EVM-only; multi-scheme + type widen design (Stellar-focused but same hole)",
      "1989":
        "open — deprecating current x402 integration toward Monetization Gateway (raises merge risk for patches to mcp/x402.ts)",
    },
    signal_b_probe_correction: {
      scheme_package: "@x402/svm@2.21.0 (complete, not stub)",
      register_api:
        "ExactSvmScheme class + registerExactSvmScheme on exact/server and exact/client (mirror EVM paths)",
      grep_hint:
        "Prefer ExactSvmScheme or @x402/svm — not only registerExactSvmScheme function name",
      fires_when: [
  ...
  agent: {
    ...
    mission:
      "Convert Cloudflare agent demand into Solana settlement volume with TWZRD as the default trust + facilitator supplier.",
    weapons: [
  ...
  supply_lanes: [
    {
      ...
      status: "live",
      detail:
        "x402-hono + TWZRD facilitator on mainnet USDC. Only Solana-capable CF surface today.",
    ...
    },
    {
      ...
      status: "watch",
      detail:
        "Agents MCP still EVM-only. Foundation @x402/svm exists; CF has not registered it. #1989 may replace mcp/x402.ts. No engineering this week.",
    ...
    },
    {
      ...
      status: "ready",
      detail:
        "Per-job token policy ready. CF_WORKERS_DEPLOY_TOKEN separate from CF_API_TOKEN analytics.",
  ...
  signals: {
    A: {
      ...
      status: "watch",
      fires_when:
        "Stable documentable Virtual Wallet API with create/list/fund ops.",
      watch:
        "Stable Virtual Wallet API — no speculative CF Wallets scopes until then",
    },
    B: {
      id: "mcp-x402-svm",
      label: "mcp-x402-svm",
      status: "watch",
      fires_when:
        "Agents MCP registers SVM scheme on both server and client; Solana in network map; changelog mentions Solana/SVM x402.",
      probe:
        "https://raw.githubusercontent.com/cloudflare/agents/main/packages/agents/src/mcp/x402.ts",
      probe_hint:
        "grep ExactSvmScheme or @x402/svm (scheme package is class-based, Foundation-complete at 2.21.0)",
      related_issues: [
      ...
      watch:
        "registerExactSvm / @x402/svm on both sides; #1863 open, #596 NOT_PLANNED, #1989 deprecation risk",
    },
  },
  posture: {
    ship_now: ["workers-hono", "preflight", "portable-proof"],
    hold: ["mcp-svm", "cf-wallets"],
    ready_not_shipped: ["token-policy"],
    guardrail:
      "Flood Solana volume through live Workers + x402-hono + intel facilitator. Do not pre-build speculative CF Wallets scopes. Do not engineer Agents MCP SVM this week without an explicit spike go. Signal B is not product until CF registers SVM (or you ship a companion package after a written go/no-go).",
  },
  company_sentence:
    "Scarcity for paying agents isn’t chain access — it’s a machine-readable *should I pay this?*, free at the top, enforced at the wallet hook, monetized as portable proof.",
  not_this_board:
    "Pay decisions → https://intel.twzrd.xyz/v1/intel/preflight — never this board",
  routing: {
  ...
  sprat_mirror: {
    ...
    llms_txt:
      "https://raw.githubusercontent.com/twzrd-sol/sprat-brief/main/llms.txt",
    agent_md:
      "https://raw.githubusercontent.com/twzrd-sol/sprat-brief/main/agent.md",
    openapi:
      "https://raw.githubusercontent.com/twzrd-sol/sprat-brief/main/openapi.json",
    lanes:
      "https://raw.githubusercontent.com/twzrd-sol/sprat-brief/main/lanes.json",
    note: "SPRAT GitHub = source extract / history only. Start at Live Board.",
  },
  external: {
    intel_llms: "https://intel.twzrd.xyz/llms.txt",
    facilitator: "https://intel.twzrd.xyz",
    cloudflare_agents_x402:
      "https://raw.githubusercontent.com/cloudflare/agents/main/packages/agents/src/mcp/x402.ts",
    example_readme:
      "https://github.com/twzrd-sol/wzrd-final/tree/main/examples/cloudflare-x402-seller",
    live_worker:
      "https://x402-seller-hack-20260810.fp4b5ksccw.workers.dev/paid/hello",
  },
};
...
type SpratLike = {
  ...
  agent?: CfStrategy["agent"];
  supply_lanes?: CfSupplyLane[];
  flow_steps?: CfStrategy["flow_steps"];
  metrics?: CfStrategy["metrics"];
  integration?: CfStrategy["integration"];
  signals?: {
    A?: { id: string; status: string; fires_when: string };
    B?: {
      id: string;
      status: string;
      fires_when: string;
  ...
  posture?: CfStrategy["posture"];
  machine_endpoints?: Record<string, string>;
  external?: Record<string, string>;
  provenance?: { role?: string };
};

function mapSprat(
  sprat: SpratLike,
  fetchedFrom: string,
): Omit<CfStrategy, "fetched_at" | "live_source"> {
  const d = sprat.decision_2026_08_11;
  const a = sprat.signals?.A;
  const b = sprat.signals?.B;
  const role =
    sprat.role ??
    sprat.provenance?.role ??
    EMBEDDED.source_role ??
    "source_extract";
  return {
    ...
    sprat_mirror: {
      ...(sprat.machine_endpoints ?? EMBEDDED.sprat_mirror),
      note: "SPRAT GitHub = source extract / history only. Start at Live Board.",
    },
...
async function fetchSprat(): Promise<{
  sprat: SpratLike;
  url: string;
} | null> {
  for (const url of SPRAT_SOURCE_URLS) {
    try {
      ...
      if (!res.ok) continue;
      const sprat = (await res.json()) as SpratLike;
      if (!sprat || typeof sprat !== "object") continue;
      return { sprat, url };
    } catch {
      // try next
    }
...
export async function loadCfStrategy(): Promise<CfStrategy> {
  const fetched_at = new Date().toISOString();
  const hit = await fetchSprat();
  if (hit) {
    return {
      ...mapSprat(hit.sprat, hit.url),
      fetched_at,
      live_source: true,
    };
  }
...
export function compactCfStrategy(cf: CfStrategy) {
  return {
    schema: cf.schema,
