...
- **Brand assets — a custom share card is the default**: open the **`og`**
  ...
  game card — see the og skill; not `twitter:card`, not a custom `x:` type).
...
og:        keep og:image in root head; svg favicon always; custom og.jpg card is the DEFAULT (DOM games too; plain utilities keep placeholder); games → og:type="x:game" (X game-card unfurl; not twitter:card / x:type); brand assets in a parallel `task` by default → .grok/skills/og/SKILL.md
